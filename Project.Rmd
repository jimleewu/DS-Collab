...
When comparing the models on the cross-validated data, we can see that the Random Forest model performed the best with a ROC_AUC of 0.8023810 Specifically, the model the random forest model with hyper parameters mtry = 2, trees = 500, and min_n	= 15 performed the best with the ROC_AUC of 0.8023810
...
```
Four our ROC_AUC, we can see that our model performed generally well. The graph looks good, as we want the curve to follow a trajectory that is as up and to the left as possible
